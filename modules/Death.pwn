...
MakePlayerWounded(playerid, issuerid = INVALID_PLAYER_ID, bool:car_death = false, reason = 0)
{
	...
	{
	    SendClientMessage(playerid, COLOR_GRAD2, "[ ! ] Você desligou.");
	...
	if(brutally_wounded && !ForceKnockout{playerid})
	{
		...
		for(new i = 0; i < MAX_PLAYER_WEAPON_PACKAGE; ++i)
		{
		    if(i < 3)
		    {
				...
				{
					if(!lost_weapons)
						format(weap_str, 256, "%s[%d Munição]", ReturnWeaponName(PlayerData[playerid][pWeapon][i]), PlayerData[playerid][pAmmunation][i]);
					else
						format(weap_str, 256, "%s, %s[%d Munição]", weap_str, ReturnWeaponName(PlayerData[playerid][pWeapon][i]), PlayerData[playerid][pAmmunation][i]);
			...
			if(PlayerData[playerid][pPackageWP][i] > 0)
			{
			    if(!lost_packages)
			    {
			        format(packages_str, 256, "%s[%d Munição]", GetWeaponPackageName(PlayerData[playerid][pPackageWP][i]), PlayerData[playerid][pPackageAmmo][i]);
				}
				else
				{
				    if(lost_packages == 4 || lost_packages == 8)
						format(packages_str, 256, "%s\n%s[%d Munição]", packages_str, GetWeaponPackageName(PlayerData[playerid][pPackageWP][i]), PlayerData[playerid][pPackageAmmo][i]);
				    else
						format(packages_str, 256, "%s, %s[%d Munição]", packages_str, GetWeaponPackageName(PlayerData[playerid][pPackageWP][i]), PlayerData[playerid][pPackageAmmo][i]);
				}
		...
		SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "EMT: Suas contas médicas são $%d", bills);
		...
		if(lost_weapons || lost_packages || lost_drugs)
		{
		    if(lost_weapons)
		    {
				...
				format(body, sizeof(body), "{FF6347}[ ! ]{FFFFFF} Você perdeu armas:\n%s", weap_str);
			}

		    if(lost_packages)
		    {
			    if(lost_weapons)
					format(body, sizeof(body), "%s\n\n{FF6347}[ ! ]{FFFFFF} Você perdeu seus pacotes de armas:\n%s", body, packages_str);
			    else
					format(body, sizeof(body), "{FF6347}[ ! ]{FFFFFF} Você perdeu seus pacotes de armas:\n%s", packages_str);
			}

			if(lost_drugs)
			{
			    if(lost_weapons || lost_packages)
					format(body, sizeof(body), "%s\n\n{FF6347}[ ! ]{FFFFFF} Você perdeu suas drogas:\n%s", body, drug_str);
			    else
					format(body, sizeof(body), "{FF6347}[ ! ]{FFFFFF} Você perdeu suas drogas:\n%s", drug_str);
			}
			...
			format(body, sizeof(body), "%s\n\n{FFFFFF}Você perdeu seus itens em: {FF6347}%d/%d/%d -- %d:%d:%d", body, day, month, year, hour, minute, second);

			SetPVarString(playerid, "LostItems", body);

			SendNoticeMessage(playerid, "Você morreu e perdeu seus itens. Use /lostitems para ver o que você perdeu.");
		}

		SendClientMessage(playerid, COLOR_LIGHTRED, "Você foi brutalmente ferido, agora se um médico ou qualquer outra pessoa não te salvar, você morrerá.");
		SendClientMessage(playerid, COLOR_LIGHTRED, "Para aceitar a morte, digite /aceitarmorte.");

		format(sgstr, sizeof(sgstr), "(( Foi ferido %d vezes, /ferimentos %d para mais informações. ))", TotalPlayerDamages[playerid], playerid);
		SetPlayerChatBubble(playerid, sgstr, 0xFF6347FF, 30.0, 300 * 1000);
		SendClientMessageEx(playerid, COLOR_LIGHTRED, sgstr);

		if(headshot_wound)
		{
		    SendClientMessage(playerid, COLOR_YELLOW, "-> Você sofreu um tiro na cabeça e morreu.");
		    SendClientMessage(playerid, COLOR_YELLOW, "-> Você precisa esperar 60 segundos e então você podera usar /respawnme.");
	...
	else
	{
		...
		SendClientMessage(playerid, COLOR_LIGHTRED, "Você foi nocauteado!");
		SendClientMessage(playerid, COLOR_LIGHTRED, "Se alguém não vier ajudá-lo, ou um paramédico não salvá-lo, você pode morrer.");
		SendClientMessage(playerid, COLOR_LIGHTRED, "Você não perde nenhum dos seus itens, pois você só fica inconsciente até morrer.");
		SendClientMessage(playerid, COLOR_LIGHTRED, "Para aceitar a morte, digite /aceitarmorte.");

		format(sgstr, sizeof(sgstr), "(( Foi nocauteado e atingido %d vezes, /levantar %d para ajudá-lo a levantar. ))", TotalPlayerDamages[playerid], playerid);
...
MakePlayerDead(playerid, bool:executed = true)
{
	...
	SetPlayerChatBubble(playerid, "(( ESTE JOGADOR ESTÁ MORTO. ))", 0xFF6347FF, 10.0, 300 * 1000);

	if(executed) SendClientMessage(playerid, COLOR_YELLOW, "-> Agora você está morto. Você precisa esperar 60 segundos e então você podera usar /respawnme.");

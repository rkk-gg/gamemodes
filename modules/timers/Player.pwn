ptask Players[1000](i)
{
	    ...
	    if(SafeTime[i] > 0)
		{
  			if(FinishedDownloadingModels{i} == true)
			{
				...
				if(SafeTime[i] == 1)
				{
					...
					SendClientMessage(i, COLOR_LIGHTRED, "Você foi kickado por não conseguir fazer login em 60 segundos."), KickEx(i);
				}
			}
		}
	...
	if(AFKCount[i] >= 600)
	{
		...
		SendClientMessage(i, COLOR_LIGHTRED, "Você foi kickado pelo SISTEMA, motivo: AFK");
	...
	if(PlayerData[i][pExpCounter] == 3600)
	{
		...
		if(PlayerData[i][pExp] >= expamount && (PlayerData[i][pLevel] < 2 || PlayerData[i][pDonateRank]))
		{
			...
			format(sgstr, sizeof(sgstr), "~g~Parabens~n~~w~Voce subiu para o level %i", PlayerData[i][pLevel]);
    	...
    	if(!KnockedOut{i})
		{
		    if(DeathTimer[i] > 0)
		    {
				...
				else
				{
					if(DeathTimer[i] <= 0)
					{
						SendClientMessage(i, COLOR_YELLOW, "-> Seu tempo de morte acabou. Você já pode utilizar /respawnme.");
					}
				}
			}
		}
	...
	if(PlayerData[i][pCellTime] > 0)
	{
		if(PlayerData[i][pCallLine] != INVALID_PLAYER_ID)
		{
			...
			if(PlayerData[i][pCellTime] == cchargetime)
			{
				...
				if(PlayerData[calling][pCallLine] == i && !PlayerData[i][pIncomingCall])
				{
					if(PlayerData[i][pCash] - (PlayerData[i][pCallCost] + callcost) < 0)
					{
						...
						format(sgstr, sizeof(sgstr), "~w~A chamada custou~n~~r~$%d", PlayerData[i][pCallCost]);
						GameTextForPlayer(i, sgstr, 5000, 1);

						PlayerData[i][pCallCost] = 0;

						SendClientMessage(calling,  COLOR_GRAD2, "[ ! ] Eles desligaram.");
			...
			if(PlayerData[calling][pIncomingCall] && PlayerData[i][pCellTime] % 10 == 1 && !ph_silentmode[calling])
			{
				...
				if(PlayerData[i][pCellTime] == 10) AnnounceMyAction(calling, "o telefone começa a tocar.");
			}
		}
	}
	else
	    ...
	    switch(PlayerData[i][pJailed])
	    {
		    ...
		    case 2, 3:
		    {
		        if(PlayerData[i][pSentenceTime] > 0)
				{
			        ...
			        if(!PlayerData[i][pSentenceTime])
			        {
						SendClientMessage(i, COLOR_YELLOW, "-> Seu tempo de prisão acabou, use /liberdade to get released.");
			        }
				}
	        }
	    }
		...
		if(CoreVehicles[vehicleid][vehicleBadlyDamage])
		{
		  	...
		  	if(Keys == 8)
			{
				if(random(100) <= 10)
				{
					...
					GameTextForPlayer(i, "~g~Motor ligado", 2000, 4);
			...
			{
				GameTextForPlayer(i, "~r~Motor engasgado", 2000, 4);
		...
		if(CoreVehicles[vehicleid][startup_delay] > 0)
		{
			if(CoreVehicles[vehicleid][startup_delay_sender] != INVALID_PLAYER_ID)
			{
				...
				if(CoreVehicles[vehicleid][startup_delay] <= 0 && i == CoreVehicles[vehicleid][startup_delay_sender])
				{
					...
					{
						GameTextForPlayer(CoreVehicles[vehicleid][startup_delay_sender], "~r~Motor Engasgado", 2000, 4);
						//GameTextForPlayer(CoreVehicles[vehicleid][startup_delay_sender], "~r~Engine Refuse", 2000, 4);
					}
					else
					{
						SetEngineStatus(vehicleid, true);
						GameTextForPlayer(CoreVehicles[vehicleid][startup_delay_sender], "~g~Motor Ligado", 2000, 4);
					}
		...
		if(hp < CoreVehicles[vehicleid][vehCrash])
		{
			...
			if(CoreVehicles[vehicleid][vehicleEngineStatus])
			{
				if((hp >= 550 && hp <= 649) || (hp >= 325 && hp <= 389))
				{
					...
					SendClientMessage(i, COLOR_LIGHTRED, "A BATERIA DESLIGOU O MOTOR: O veículo está muito danificado.");
			  		SendClientMessage(i, COLOR_LIGHTRED, "DICA: É recomendável que você conserte o veículo.");
					SendClientMessage(i, COLOR_LIGHTRED, "DICA: Você pode digitar /motor para reiniciá-lo.");
				}
			}
			...
			if(vehicle_health_loss > 5)
			{
				if(LessonStarted{i})
				{
					if(GetPVarInt(i, "LessonSeconds") != 0 && GetPVarInt(i, "InDriveTest"))
					{
						...
						SendClientMessage(i, COLOR_GREEN, "Os testes de veículos falharam.");
				...
				if(CountVehiclePlayers(vehicleid) > 0)
				{
					...
					foreach (new p : VehicleOccupant(vehicleid))
					{
						...
						else
						{
							...
							SendClientMessageEx(i, COLOR_YELLOW, "%s não tinha dinheiro suficiente para pagar a corrida.", ReturnName(p, 0));
							SendClientMessage(p, COLOR_YELLOW, "Você não tinha dinheiro suficiente para pagar a corrida.");
						}
					}
				}
			}
	...
	if(RepairTime{i})
	{
	    if(!IsPlayerInAnyVehicle(i) && serviced[i])
		{
			    ...
			    if(playerState == PLAYER_STATE_ONFOOT)
				{
					...
					if(HoldingKey(i, KEY_FIRE) && GetPlayerWeapon(i) == 41 || serviced[i] == 3 || serviced[i] == 4)
					{
						switch(serviced[i])
						{
							case 1:
							{
								format(sgstr, sizeof(sgstr), "~h~~p~REPARANDO VEÍCULO.~n~~w~TOWTRUCK TEM ~b~%d~w~ PRODUTOS.", CarData[ serviceTowtruck[i] ][carComps] - (serviceComp[i] - RepairTime{i}) );
							...
							case 2:
							{
								format(sgstr, sizeof(sgstr), "~h~~p~REPARANDO CARROCERIA.~n~~w~TOWTRUCK TEM ~b~%d~w~ PRODUTOS.", CarData[ serviceTowtruck[i] ][carComps] - (serviceComp[i] - RepairTime{i}) );
								ShowPlayerFooter(i, sgstr, -1);
							...
							case 3:
							{
								ShowPlayerFooter(i, "~h~~p~REALIZANDO MANUTENÇÃO....~n~~w~((USE SUA IMAGINAÇÃO E FAÇA ROLEPLAY.))", -1);
							...
							case 5:
							{
								format(sgstr, sizeof(sgstr), "~h~~p~PINTANDO O CARRO.~n~~w~~b~%d~w~ PRODUTOS DEIXADOS NO CAMINHÃO.", CarData[ serviceTowtruck[i] ][carComps] - (serviceComp[i] - RepairTime{i}));
								ShowPlayerFooter(i, sgstr, -1);
			...
			{
				ShowPlayerFooter(i, "~h~~p~VOCÊ PRECISA ESTAR PERTO DO SEU TOW TRUCK.", -1);
			}
		}
	}
	...
	if(Unpackaging{i})
	{
		...
		if(playerPos[0] != PlayerData[i][pPos][0] || playerPos[1] != PlayerData[i][pPos][1] || playerPos[2] != PlayerData[i][pPos][2])
		{
			...
			SendNoticeMessage(i, "Você se moveu, portanto a operação foi cancelada.");
		}
  	}
...
PayDay()
{
	...
	foreach (new i : Player)
	{
		...
		if(PlayerData[i][pID] != -1 && PlayerData[i][pLevel] > 0 && PlayerData[i][pPayDay] >= 900) // 15 minutes
		{
			...
			if(house != -1)
			{
			    if(PropertyData[house][hOwnerSQLID] == PlayerData[i][pID]) // Owner
			    {
					...
					else
					{
						...
						SendClientMessage(i, COLOR_WHITE, "Você foi despejado");
					}
				}
				else
				{
				...
				SendClientMessageEx(i, COLOR_WHITE, "Aluguel pago: $%d. (Fora do seu banco)", ebill);

				if(evicted) SendClientMessage(i, COLOR_LIGHTRED, "$0{FFFFFF} dívida paga com seu salário");
			}

			SendClientMessage(i, COLOR_WHITE, "|___ EXTRATO BANCÁRIO ___|");

			SendClientMessageEx(i, COLOR_GRAD1, "   Saldo: $%d", account);
			SendClientMessageEx(i, COLOR_GRAD3, "   Taxa de Juros: %.2f", tmpintrate);
			SendClientMessageEx(i, COLOR_GRAD3, "   Juros Ganhos %s", FormatNumber(interest));
			//SendClientMessageEx(i, COLOR_GRAD3, "   Interest Gained %s", FormatNumber(interest));
			SendClientMessageEx(i, COLOR_GRAD3, "   Impostos pagos %s", FormatNumber(TaxValue));
			...
			if(PlayerData[i][pFaction] != -1)
			{
				switch(type)
				{
				    case FACTION_POLICE:
				    {
						...
						SendClientMessageEx(i, COLOR_GRAD3, "   LSPD Pagou %s", FormatNumber(2500));
				    }
				    case FACTION_MEDIC:
				    {
					    PlayerData[i][pPayCheck] += 2500;

						SendClientMessageEx(i, COLOR_GRAD3, "   LSFD Pagou %s", FormatNumber(2500));
				    }
				    case FACTION_SHERIFF:
				    {
					    PlayerData[i][pPayCheck] += 2500;

						SendClientMessageEx(i, COLOR_GRAD3, "   SASD Pagou %s", FormatNumber(2500));
				    }
				}
			}
			...
			if(PlayerData[i][pSavingsCollect])
			{
				SendClientMessageEx(i, COLOR_GRAD5, "   Rendimento dos savings: %s, at rate: 0.5", FormatNumber(savings));
				SendClientMessageEx(i, COLOR_GRAD5, "   Novo saldo dos savings: %s", FormatNumber(PlayerData[i][pSavingsCollect]));
			}

			SendClientMessage(i, COLOR_GRAD4, "|________________________|");

			SendClientMessageEx(i, COLOR_GRAD5, "   Novo Saldo: %s", FormatNumber(PlayerData[i][pAccount]));
            SendClientMessageEx(i, COLOR_GRAD5, "   Aluguel: -$%d", ebill);
			...
			if(house != -1 && evicted)
			{
			    if(PropertyData[house][hOwnerSQLID] != PlayerData[i][pID])
			    {
					...
					SendClientMessage(i, COLOR_LIGHTRED, "Você foi despejado de sua casa alugada por falta de pagamento.");
				}
			}
			...
			if(PlayerData[i][pJob] == JOB_MECHANIC || PlayerData[i][pSideJob] == JOB_MECHANIC)
			{
                ...
                SendClientMessage(i, COLOR_GRAD5, "Você recebeu $1,250 por seus serviços de mecânico.");
			}
			...
			if(PlayerData[i][pLevel] == 1)
			{
				...
				SendClientMessage(i, COLOR_GRAD5, "(( Você recebeu $2,000 por estar no nível 1. ))");
			}
			...
			if(PlayerData[i][pLevel] == 2)
			{
				...
				SendClientMessage(i, COLOR_GRAD5, "(( Você recebeu $1,500 por estar no nível 2. ))");
			}
			...
			format(string, sizeof(string), "~y~Dia de Pagamento~n~~w~Salario~n~~g~$%d", PlayerData[i][pPayCheck]);
		...
		{
			SendClientMessage(i, COLOR_GRAD5, "Você não jogou o suficiente, então não há pagamento para você.");
		}
	}

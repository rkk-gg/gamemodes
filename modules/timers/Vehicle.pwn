			...
			if(AdvertData[vehicleid][ad_id])
			{
				...
				if(AdvertData[vehicleid][ad_time] <= 0)
				{
				    if(AdvertData[vehicleid][ad_type])
					{
						...
						{
							format(vgstr, sizeof(vgstr), "[Anúncio da empresa] %.70s ...", AdvertData[vehicleid][ad_text]);
							SendClientMessageToAll(COLOR_GREEN, vgstr);
							format(vgstr, sizeof(vgstr), "[Anúncio da empresa] ... %s", AdvertData[vehicleid][ad_text][70]);
							SendClientMessageToAll(COLOR_GREEN, vgstr);
						}
						else
						{
							format(vgstr, sizeof(vgstr), "[Anúncio da empresa] %s", AdvertData[vehicleid][ad_text]);
							SendClientMessageToAll(COLOR_GREEN, vgstr);
						}
				    }
				    else
					{
						if(strlen(AdvertData[vehicleid][ad_text]) > 70)
						{
							format(vgstr, sizeof(vgstr), "[Anúncio] %.70s ...", AdvertData[vehicleid][ad_text]);
							SendClientMessageToAll(COLOR_GREEN, vgstr);
							format(vgstr, sizeof(vgstr), "[Anúncio] ... %s [PH: %d]", AdvertData[vehicleid][ad_text][70], PlayerData[AdvertData[vehicleid][ad_owner]][pPnumber]);
							SendClientMessageToAll(COLOR_GREEN, vgstr);
						}
						else
						{
							format(vgstr, sizeof(vgstr), "[Anúncio] %s [PH: %d]", AdvertData[vehicleid][ad_text], PlayerData[AdvertData[vehicleid][ad_owner]][pPnumber]);
							SendClientMessageToAll(COLOR_GREEN, vgstr);
						}
				    }
			...
			switch(VehicleLabel[vehicleid][vLabelType])
			{
			    case VLT_TYPE_TOWING:
				{
				    ...
				    if(VehicleLabel[vehicleid][vLabelCount] >= VehicleLabel[vehicleid][vLabelTime])
					{
						...
						{
						    SendClientMessageEx(targetid, COLOR_RADIO, "%s foi rebocado para seu local de estacionamento.", g_arrVehicleNames[GetVehicleModel(vehicleid) - 400]);
				...
				case VLT_TYPE_REFILL:
				{
					...
					if(dealership)
					{
					    ...
					    if(VehicleLabel[vehicleid][vLabelCount] >= VehicleLabel[vehicleid][vLabelTime])
						{
							...
							SendClientMessageEx(targetid, COLOR_RADIO, "Esta operação requer $%d", uprice);
						...
						for(new x = 10; x > 0; --x)
						{
	     				...
	     				switch(VehicleLabel[vehicleid][vLabelType])
	     				{
							case VLT_TYPE_UPGRADELOCK:
								format(vgstr, sizeof(vgstr), "(( %s ))\nATUALIZANDO TRAVA", labelstring);
							case VLT_TYPE_UPGRADEIMMOB:
								format(vgstr, sizeof(vgstr), "(( %s ))\nATUALIZANDO IMOBILIZADOR", labelstring);
							case VLT_TYPE_UPGRADEALARM:
								format(vgstr, sizeof(vgstr), "(( %s ))\nATUALIZANDO ALARME", labelstring);
							case VLT_TYPE_UPGRADEINSURANCE:
								format(vgstr, sizeof(vgstr), "(( %s ))\nATUALIZANDO SEGURO", labelstring);
							case VLT_TYPE_UPGRADEBATTERY:
								format(vgstr, sizeof(vgstr), "(( %s ))\nSUBSTITUINDO A BATERIA", labelstring);
							case VLT_TYPE_UPGRADEENGINE:
								format(vgstr, sizeof(vgstr), "(( %s ))\nTROCANDO O MOTOR", labelstring);
						}
			     				...
			     				switch(VehicleLabel[vehicleid][vLabelType])
			     				{
									...
									case VLT_TYPE_UPGRADEENGINE:
									{
										...
										if(PlayerData[targetid][pCash] >= uprice)
										{
					                     	...
					                     	ShowPlayerFooter(targetid, "~w~MOTOR SUBSTITUÍDO!", 10000);
									   	}
									}
								}
				...
				case VLT_TYPE_REGISTER:
				{
					...
					if(dealership)
					{
						...
						format(vgstr, sizeof(vgstr), "(( %s ))\nRegistrando veículo", labelstring);
			            UpdateDynamic3DTextLabelText(VehicleLabel[vehicleid][vLabel], COLOR_GREEN2, vgstr);
					    ...
					    if(VehicleLabel[vehicleid][vLabelCount] >= VehicleLabel[vehicleid][vLabelTime])
						{
							...
							{
	                            SendClientMessage(targetid, COLOR_YELLOW3, "Esta operação requer $100");
						   	...
						   	else SendClientMessage(targetid, COLOR_YELLOW3, "Esta operação requer $100");
						}
					}
					else
					{

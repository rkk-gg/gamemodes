...
SetVehicleLabel(vehicleid, type, time)
{
    if(!IsValidDynamic3DTextLabel(VehicleLabel[vehicleid][vLabel]))
	{
		switch(type)
		{
		    case VLT_TYPE_TOWING: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("(( ---------- ))\nREBOCANDO VEÍCULO", COLOR_GREEN2, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 100);
		    case VLT_TYPE_PERMITFACTION: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("você não tem permissão para entrar neste veículo (Faction-vehicle)", COLOR_WHITE, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 50);
		    case VLT_TYPE_LOCK: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("Este veículo está trancado!", 0xFF6347FF, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 50);
		    case VLT_TYPE_UNREGISTER: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("(( ---------- ))\nREMOVENDO REGISTRO", COLOR_GREEN2, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 50);
			case VLT_TYPE_REGISTER: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("(( ---------- ))\nREGISTRANDO VEÍCULO", COLOR_GREEN2, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 50);
			...
			case VLT_TYPE_UPGRADELOCK: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("(( ---------- ))\nATUALIZANDO TRAVA", COLOR_GREEN2, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 50);
			case VLT_TYPE_UPGRADEIMMOB: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("(( ---------- ))\nATUALIZANDO IMOBILIZADOR", COLOR_GREEN2, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 50);
			case VLT_TYPE_UPGRADEALARM: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("(( ---------- ))\nATUALIZANDO ALARME", COLOR_GREEN2, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 50);
			case VLT_TYPE_UPGRADEINSURANCE: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("(( ---------- ))\nATUALIZANDO SEGURO", COLOR_GREEN2, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 50);
			case VLT_TYPE_ARMOUR: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("(( ---------- ))\nATUALIZANDO ARMOUR", COLOR_GREEN2, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 50);
			case VLT_TYPE_OPERAOUTOFRANG: VehicleLabel[vehicleid][vLabel] = CreateDynamic3DTextLabel("(( A OPERAÇÃO FALHOU ))", 0xFF6347FF, 0, 0, 0, 20, INVALID_PLAYER_ID, vehicleid,  0, 0, 0, -1, 50);
...
{
    SendClientMessageEx(playerid, COLOR_CYAN, "Segurança: Trava[%d], Alarme[%d], Imobilizador[%d], Seguro[%d]", CarData[carid][carLock], CarData[carid][carAlarm], CarData[carid][carImmob], CarData[carid][carInsurance]);
	SendClientMessageEx(playerid, COLOR_CYAN, "Vida Útil: Vida do Motor[%.2f], Vida da Bateria[%.2f], Quilometro Rodados[%.2f]", CarData[carid][carEngineL], CarData[carid][carBatteryL], CarData[carid][carMileage]);

	if(CarData[carid][carArmour])
	{
		SendClientMessageEx(playerid, COLOR_CYAN, "Outros: Cor primária[{%06x}#%d{F0F8FF}], Cor Secundária[{%06x}#%d{F0F8FF}], Placa[%s], Armour[%.2f]", \
	...
	{
		SendClientMessageEx(playerid, COLOR_CYAN, "Outros: Cor primária[{%06x}#%d{F0F8FF}], Cor Secundária[{%06x}#%d{F0F8FF}], Placa[%s]", \
...
PutPlayerSettingVehicle(playerid, model, price)
{
    if(CountVehicleKeys(playerid) >= MaxVehiclesSpawned(playerid))
		return SendServerMessage(playerid, "Você já tem o máximo de veículos spawnados.");

	if(PlayerData[playerid][pCash] < price)
		return SendServerMessage(playerid, "Você não tem dinheiro suficiente para este veículo.");

	if(PlayerData[playerid][pDonateRank] < IsDonateCar(model))
		return SendServerMessage(playerid, "Este veículo é somente para doadores.");

	if(IsCopCar(model))
	{
		if(GetFactionType(playerid) != 1)
			return SendServerMessage(playerid, "Você não faz parte desta facção (veículo-fac).");
	}
...
ShowPlayerDealercarDialog(playerid)
{
	...
	if(VDealerLock[playerid])
	{
		...
		format(string2, sizeof(string2), "%s\n{FFFF00}Trava:\t\t{B4B5B7} level %d\t{FFFFFF}%s", string2,
	...
	if(VDealerAlarm[playerid])
	{
		...
		format(string2, sizeof(string2), "%s\n{FFFF00}Alarme:\t\t{B4B5B7} level %d\t{FFFFFF}%s", string2,
	...
	if(VDealerImmob[playerid])
	{
		...
		format(string2, sizeof(string2), "%s\n{FFFF00}Imobilizador:\t{B4B5B7} level %d\t{FFFFFF}%s", string2,
...
Dialog:Dealercar(playerid, response, listitem, inputtext[])
{
	...
	{
		format(string, sizeof(string), "%s - %s", g_arrVehicleNames[model - 400], GetDealershipVehiclePrice(playerid));
	    Dialog_Show(playerid, DealercarConfirm, DIALOG_STYLE_MSGBOX, string, "Tem certeza de que deseja comprar este veículo?", "Comprar", "Cancelar");
	}
	else
	{
		format(string, sizeof(string), "%s - %s", g_arrVehicleNames[model - 400], GetDealershipVehiclePrice(playerid));
	    Dialog_Show(playerid, DealerMenu, DIALOG_STYLE_LIST, string, "Alarme\nTrava\nImobilizador\nCores\n%s", "Acrescentar", "<<", (VDealerXM[playerid]) ? ("{F7FE2E}XM-Radio Instalado") : ("Sem XM instalado"));
	}
	return true;
}
	...
	if(response)
	{
	    switch(listitem)
	    {
	        case 0:
	        {
				...
				Dialog_Show(playerid, CarDealerAlarm, DIALOG_STYLE_LIST, "Alarme", "%sNone\n%sAlarm Level 1 - %s\n%sAlarm Level 2 - %s\n%sAlarm Level 3 - %s\n%sAlarm Level 4 - %s", "Modify", "<<",
	        ...
	        case 1:
	        {
				...
				Dialog_Show(playerid, CarDealerLock, DIALOG_STYLE_LIST, "Trava", "%sLock Level 1 - $0\n%sLock Level 2 - %s\n%sLock Level 3 - %s\n%sLevel 4 - %s\n%sLevel 5 - %s", "Modify", "<<",
	        ...
	        case 4:
	        {
			    ...
			    Dialog_Show(playerid, DealerMenu, DIALOG_STYLE_LIST, string, "Alarme\nTrava\nImobilizador\nCores\n%s", "Acrescentar", "<<", (VDealerXM[playerid]) ? ("{F7FE2E}XM-Radio Installed") : ("No XM Installed"));
	        }
	    }
	}
...
public OnQueryBuyVehicle(playerid, carid, model, price)
{
	...
	SendClientMessageEx(playerid, COLOR_WHITE, "Bem-vindo ao seu %s", g_arrVehicleNames[model - 400]);
	SendClientMessageEx(playerid, COLOR_CYAN, "Trava[%d], Alarme[%d], Imobilizador[%d], Seguro[%d]", CarData[carid][carLock], CarData[carid][carAlarm], CarData[carid][carImmob], CarData[carid][carInsurance]);
	SendClientMessageEx(playerid, COLOR_CYAN, "Vida Útil: Vida do Motor[%.2f], Vida da Bateria[%.2f], Quilometro Rodados[%.2f]", CarData[carid][carEngineL], CarData[carid][carBatteryL], CarData[carid][carMileage]);
...
public DuplicatePlates(playerid, id, plate[])
{
    ...
    else
	{
		...
		SendClientMessage(playerid, 0xADFF2FFF, "PROCESSADO: Lista reconstruída.");
...
SpawnPlayerCar(playerid, slot)
{
	if(PCarOpening{playerid})
	{
		...
		for(new i = 0; i < MAX_VEHICLE_KEYS; ++i)
		{
		    if(PlayerData[playerid][pCarKeys][i] != 9999)
			{
				if(CarData[ PlayerData[playerid][pCarKeys][i] ][carID] == VehicleCache[playerid][cacheID])
					return SendServerMessage(playerid, "Este veículo já foi spawnado.");
        ...
        if(count >= MaxVehiclesSpawned(playerid))
			return SendServerMessage(playerid, "Você já tem sua quantidade máxima de veículos spawnados.");
...
FUNX::OnPlayerVehicleSpawn(playerid)
{
	if(PCarOpening{playerid})
	{
		...
		if(i != -1)
		{
			...
			SendClientMessageEx(playerid, COLOR_GREEN, "%s foi spawnado em seu estacionamento:", ReturnVehicleName(CarData[i][carVehicle]));
            SendClientMessageEx(playerid, COLOR_CYAN, "Trava[%d], Alarme[%d], Imobilizador[%d], Seguro[%d]", CarData[i][carLock], CarData[i][carAlarm], CarData[i][carImmob], CarData[i][carInsurance]);
			SendClientMessageEx(playerid, COLOR_CYAN, "Vida Útil: Vida do Motor[%.2f], Vida da Bateria[%.2f], Quilometro Rodados[%.2f]", CarData[i][carEngineL], CarData[i][carBatteryL], CarData[i][carMileage]);
			SendClientMessage(playerid, TEAM_CUN_COLOR, "Dica: Siga o checkpoint vermelho para encontrar seu veículo.");

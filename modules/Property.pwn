...
Property_Refresh(houseid)
{
	if(!PropertyData[houseid][hOwned])
	{
	    if(!IsValidDynamic3DTextLabel(Text3D:PropertyData[houseid][hLabel]))
		{
	        ...
	        {
	            format(sgstr, sizeof(sgstr), ""EMBED_YELLOW"Casa[%d] A venda:\nPreço: $%d\nLevel: %d", houseid, PropertyData[houseid][hPrice], PropertyData[houseid][hLevelbuy]);
			...
			{
				format(sgstr, sizeof(sgstr), ""EMBED_YELLOW"Apartamento[%d] A venda:\nPreço: $%d\nLevel: %d", houseid, PropertyData[houseid][hPrice], PropertyData[houseid][hLevelbuy]);
...
Complex_Refresh(id)
{
	if(id != -1)
	{
		...
		if(!ComplexData[id][aOwned])
		{
			...
			{
			    format(sgstr, sizeof(sgstr), "{C38A39}Complexo de Apartamentos[%d] A venda:\nPreço: $%d\nLevel: %d\nTrava Level:%d\nAlarme Level:%d", id, ComplexData[id][aPrice], ComplexData[id][aLevelbuy], ComplexData[id][aLocklevel], ComplexData[id][aAlarmlevel]);

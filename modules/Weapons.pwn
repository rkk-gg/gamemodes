...
GivePlayerValidWeapon(playerid, weaponid, ammo, license = 0, bool:notif = true, bool:replace = true)
{
	...
	if(IsMelee(weaponid))
	{
		...
		if(notif) SendClientMessageEx(playerid, COLOR_GREEN, "[Arma branca] Agora você irá spawnar com um(a) %s", ReturnWeaponName(weaponid));
	}
	else if(IsPrimary(weaponid))
	{
		...
		if(notif) SendClientMessageEx(playerid, COLOR_GREEN, "[Arma primária] Agora você irá spawnar com um(a) %s", ReturnWeaponName(weaponid));
	...
	else if(IsSecondary(weaponid))
	{
		...
		if(notif) SendClientMessageEx(playerid, COLOR_GREEN, "[Arma secundária] Agora você irá spawnar com um(a) %s", ReturnWeaponName(weaponid));
...
CallbackDamages(playerid, issuerid, bodypart, weaponid, Float:amount, Float:armour)
{
	...
	//SQL_LogAction(extraid, "Attacked %s with %s in %s (%d)", ReturnName(playerid), (weaponid), ReturnBodypartName(bodypart), DamageInfo[playerid][id][eDamageArmor]);
	return true;
}

CMD:respawnrental(playerid, params[])
{
	if(PlayerData[playerid][pAdmin] < 1)
		return SendUnauthMessage(playerid, "Unauthorized."); //O código 51 é um código de erro que indica que a transação com cartão de crédito não foi autorizada. 
...
CMD:createcar(playerid, params[])
{
 	if(PlayerData[playerid][pAdmin] < 4)
    	return SendErrorMessage(playerid, "Voc� n�o est� autorizado a usar este comando.");
...
YCMD:reportar(playerid, params[], help) = re;
...
CMD:re(playerid, params[])
{
	...
	if(sscanf(params, "s[128]S()[128]", inputOne, inputSec))
	    return SendClientMessage(playerid, COLOR_LIGHTRED, "Report USAGE: /re [(ID Opcional/ParteDoNome)] [texto]");
	...
	{
	    if(isnull(inputSec))
	        return SendClientMessage(playerid, COLOR_LIGHTRED, "Sua mensagem � muito curta ou muito longa.");
	...
	format(reportBody, sizeof(reportBody), "{FF0000}AVISO:\n{FFFFFF}Voc� est� prestes a enviar o seguinte relat�rio para todos os administradores online: %s\n\n", PlayerData[playerid][pReportMessage]);

	strcat(reportBody, "- Relatar a��es que n�o est�o acontecendo no momento � extremamente dif�cil para os administradores online lidarem imediatamente, pois nenhuma prova � apresentada a eles..\n");
	strcat(reportBody, "� altamente recomend�vel ir at� os f�runs e enviar um relat�rio no f�rum com evid�ncias, em vez disso.\n\n");
	strcat(reportBody, "- � importante que voc� n�o envie relat�rios dentro do jogo sobre coisas que s�o melhor tratadas nos f�runs, como as mencionadas anteriormente e/ou problemas com doa��es.\n");
	strcat(reportBody, "Solicita��es de refundos ap�s rollbacks do servidor, apelos de banimento e assim por diante. Se voc� acha que sofreu DeathMatch, considere primeiro perguntar � parte respons�vel pela morte o motivo.\n\n");
	strcat(reportBody, "- Por favor, lembre-se de que, se voc� est� relatando um jogador que acredita que ele tenha violado as regras ou um bug que possa ter encontrado, as capturas de tela s�o a melhor chance de provar isso.\n");
	strcat(reportBody, "Incentivamos que voc� tire capturas de tela antes e depois de fazer o relat�rio.\n\n");
	strcat(reportBody, "- Caso voc� solicite um teletransporte, descongelamento, tapa ou qualquer outra situa��o semelhante, deve informar o motivo. De maneira geral, � necess�rio ser descritivo em seus relat�rios.\n");
	strcat(reportBody, "- Por favor, n�o envie relat�rios em excesso. Tenha paci�ncia e um administrador ir� ajud�-lo assim que poss�vel.\n");
	strcat(reportBody, "Voc� n�o deve tentar interromper ou atrasar o roleplay, a menos que esteja recebendo assist�ncia de um administrador ou o outro jogador esteja te deixando sexualmente desconfort�vel.");

	Dialog_Show(playerid, ReportConfirm, DIALOG_STYLE_MSGBOX, "Aviso, voc� est� prestes a enviar um /re", reportBody, "Continuar", "N�o enviar");
	return true;
}
...
CMD:veh(playerid, params[])
{
    ...
    SendUnauthMessage(playerid, "Voc� n�o est� autorizado a usar este comando.");
	return true;
}
...
CMD:ahelp(playerid, params[])
{
	...
	if(PlayerData[playerid][pAdmin] >= 4) // Lead Admin
	{
		...
		SendClientMessage(playerid, COLOR_GREEN, "[Level 4]: {B4B5B7}/createprop /setprop /createapt /setapt /removeapt /makebusiness /edibusiness /asellbiz /agivebiz");
	...
	if(PlayerData[playerid][pAdmin] >= 1337) // Management
	{
	    ...
	    SendClientMessage(playerid, COLOR_GREEN, "[Level 1337]: {B4B5B7}/ecmd /createatm /deleteatm /makestreet /maketele /xyz /icv");
  		...
  		SendClientMessage(playerid, COLOR_GREEN, "[Level 1337]: {B4B5B7}/createpoint /editpoint /points /particlegun");

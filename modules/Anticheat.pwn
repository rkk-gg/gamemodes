stock OnPlayerAirbreak(playerid)
{
	...
	SendAdminAlert(COLOR_YELLOW, JUNIOR_ADMINS, "AdmWarn(1): [Airbreak] %s (%d) possivelmente esta usando cheat.", ReturnName(playerid), playerid);

    SQL_LogAction(playerid, "[Airbreak] %s (%d) possivelmente esta usando cheat.", ReturnName(playerid), playerid);
	return true;
}
...
stock OnPlayerFlyHack(playerid)
{
	...
	SendAdminAlert(COLOR_LIGHTRED, JUNIOR_ADMINS, "AdmWarn(1): [FlyHack] %s (%d) possivelmente esta usando cheat.", ReturnName(playerid), playerid);

    SQL_LogAction(playerid, "[FlyHack] %s (%d) possivelmente esta usando cheat.", ReturnName(playerid), playerid);
	return true;
}

...
Business_FoodMenu(playerid)
{
	...
	if((id = Business_Inside(playerid)) != -1)
	{
		...
		else SendErrorMessage(playerid, "Voc� n�o est� em um restaurante.");
	}	
}
...
Business_PrintInfo(playerid, targetid)
{
	...
	SendClientMessageEx(playerid, COLOR_WHITE, "Endere�o: %s, San Andreas", ReturnLocation(playerid));
	...
	SendClientMessageEx(playerid, COLOR_WHITE, "Upgardes:[0] Cofre:[%s] Pre�o da entrada:[%s]", FormatNumber(BusinessData[targetid][bTill]), FormatNumber(BusinessData[targetid][bEntranceCost]));
	SendClientMessageEx(playerid, COLOR_WHITE, "Informa��es T�cnicas: 1 Carga = %d Produtos. Atualmente: Produtos[%d / %d], Pre�o da Carga[%d]", GetProductPerCargo(BusinessData[targetid][bType]), BusinessData[targetid][bProducts], BusinessData[targetid][bMaxProducts], BusinessData[targetid][bPriceProd]);

...
{
	{19317, 14500, 6, "Baixo", 0.059999, 0.023999, 0.005, 93.6, 75.3, 0.0, 1.0, 1.0, 1.0},
	{19318, 16240, 6, "Guitarra elétrica", 0.084999, 0.043999, -0.002, 82.1, 88.2999, 8.5, 1.0, 1.0, 1.0},
	{19610, 1000, 6, "Microfone", 0.052999, 0.016999, 0.006999, 0.0, 91.1, 75.7, 1.0, 1.0, 1.0}
};
...
ShowClothingDialog(playerid, page, index = 0)
{
	...
	if(page) strcat(gstr, "\n{F2EB35}<<Pagina Anterior");
	if(next_page) strcat(gstr, "\n{F2EB35}Próxima Página>>");

	cl_buyingindex[playerid] = page;

	Dialog_Show(playerid, buyClothing, DIALOG_STYLE_TABLIST, "/comprar ItemID", gstr, "Ok", "Cancelar");
	return true;
}
...
PurchaseClothing(playerid, slot)
{
	...
	SendClientMessage(playerid, COLOR_WHITE, "DICA: Use {FFFF00}SPAÇO{FFFFFF} para olhar ao redor. Pressione {FFFF00}ESC{FFFFFF} para voltar ao menu.");
	SendClientMessage(playerid, COLOR_WHITE, "Aproveite seu novo item!");
...
cl_ShowClothingMenu(playerid)
{
	...
	if(!count) return SendNoticeMessage(playerid, "Você não tem nenhum item.");

	Dialog_Show(playerid, ClothingList, DIALOG_STYLE_LIST, "Lista de Roupas", str, "Selecionar", "<< Sair");
	return true;
}
...
Dialog:ClothingList(playerid, response, listitem, inputtext[])
{
	...
	return Dialog_Show(playerid, ClothingMenu, DIALOG_STYLE_LIST, "O que você vai fazer?", "{FFFFFF}Ajustar\nTrocar bone\nTrocar Index Slot\n%s", "Selecionar", "<<", (ClothingData[playerid][id][cl_equip]) ? ("{FFFFFF}Status{FFFF00} Off") : ("{FFFFFF}Status{FFFF00} On")); // Place On Take Off
}
	...
	else
	{
		switch(listitem)
		{
		    case 0: // Adjust The Item
		    {
				...
				SendClientMessage(playerid, COLOR_WHITE, "DICA: Use {FFFF00}ESPAÇO{FFFFFF} para olhar em volta. Pressione {FFFF00}ESC{FFFFFF}para voltar ao menu.");
		    }
		    case 1: // Change Bone Slot
		    {
                Dialog_Show(playerid, ClothingBone, DIALOG_STYLE_LIST, "Mudar Bone Slot", "Coluna\nCabeça\nBraço esquerdo superior \nBraço direito superior \nMão esquerda\nMão direita\nPerna esquerda\nPerna direita\nPé esquerdo\nPé direito\nPanturrilha direita\nPanturrilha esquerda\nBraço esquerdo\nBraço direito\nClavícula esquerda\nClavícula direita\nPescoço\nMandíbula", "Selecionar", "<<");
		    }
 		    case 2: // Change Index Slot
		    {
				if(PlayerData[playerid][pDonateRank] <= 1)
					Dialog_Show(playerid, ClothingIndex, DIALOG_STYLE_LIST, "Mudar Index Slot", "Index Slot 1\nIndex Slot 2\nIndex Slot 3\nIndex Slot 4\nIndex Slot 5", "Select", "<<");
 		    ...
 		    case 3: // On-Off
		    {
				...
				if(ClothingData[playerid][id][cl_equip]) 
                {
					...
					ClothingData[playerid][id][cl_equip] = 0;
					SendClientMessageEx(playerid, COLOR_WHITE, "Você tirou seu{FFFF00} %s", ClothingData[playerid][id][cl_name]);
				}
				else
				{
				    SetPlayerAttachedObject(playerid, ClothingData[playerid][id][cl_slot], ClothingData[playerid][id][cl_object], ClothingData[playerid][id][cl_bone], ClothingData[playerid][id][cl_x], ClothingData[playerid][id][cl_y],
					ClothingData[playerid][id][cl_z], ClothingData[playerid][id][cl_rx], ClothingData[playerid][id][cl_ry], ClothingData[playerid][id][cl_rz], ClothingData[playerid][id][cl_scalex], ClothingData[playerid][id][cl_scaley], ClothingData[playerid][id][cl_scalez]);
					SendClientMessageEx(playerid, COLOR_WHITE, "Você colocou seu{FFFF00} %s", ClothingData[playerid][id][cl_name]);
...
Dialog:ClothingBone(playerid, response, listitem, inputtext[])
{
	...
	SendClientMessageEx(playerid, COLOR_WHITE, "Você mudou a peça de roupa ({FFFF00}%s{FFFFFF}) para o bone {FFFF00}%s{FFFFFF}.", ClothingData[playerid][id][cl_name], BoneName[listitem]);
    return true;
}
...
Dialog:ClothingIndex(playerid, response, listitem, inputtext[])
{
	...
	SendClientMessageEx(playerid, COLOR_WHITE, "Você mudou a peça de roupa ({FFFF00}%s{FFFFFF}) para o index {FFFF00}#%d{FFFFFF}.", ClothingData[playerid][id][cl_name], listitem + 1);
    return true;
}

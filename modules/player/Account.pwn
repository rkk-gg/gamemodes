	...
	switch(threadid)
	{
		case THREAD_FIND_ACCOUNT:
		{
		    ...
		    else
		    {
				...
				RandomLoginScreen(extraid);
		 		...
		 		SendErrorMessage(extraid, "%s não encontrado.", ReturnNameEx(extraid));

		 		SendNoticeMessage(extraid, "Tenha certeza que esta usando o nome da sua conta principal (UCP) e não a do personagem para logar!");
			  	SendNoticeMessage(extraid, "Para criar uma conta, visite https://ucp.ls-rp.com.br/"), KickEx(extraid);
			}
  		}
		case THREAD_BAN_LOOKUP:
		{
		    ...
		    if(rows)
			{
				...
				{
					SendErrorMessage(extraid, "Seu endereço de IP está banido!");

					SendNoticeMessage(extraid, "Por favor, solicite desbanimento em forum.ls-rp.com.br antes de jogar novamente.");
				}
				else
				{
					SendErrorMessage(extraid, "Um ou mais dos seus personagens está banido!");

					SendNoticeMessage(extraid, "Por favor, solicite desbanimento em forum.ls-rp.com.br antes de jogar novamente.");
				}

				SendNoticeMessage(extraid, "Por favor, solicite desbanimento em forum.ls-rp.com.br");
		    ...
		    else
			{
		        ...
		        Dialog_Show(extraid, LoginScreen, DIALOG_STYLE_PASSWORD, "Bem vindo ao Los Santos Roleplay", "Olá,\n\nPor favor, insira sua senha abaixo para acessar a seleção de personagens.\n\nSe você não está registrado no LS:RP no momento, você pode criar\n\nsua conta em ls-rp.com.br.", "Avançar", "Sair");
			}
		}
     	case THREAD_LOGIN:
   		{
    	    ...
    	    if(!rows)
    	    {
				...
				{
					SendClientMessage(extraid, COLOR_LIGHTRED, "[SERVER]: Você foi expulso por tentativas de senha inválidas.");
				...
				format(str, sizeof(str), "Olá,\n\nPor favor, insira sua senha abaixo para acessar a seleção de personagens.\n\nSe você não está registrado no LS:RP no momento, você pode criar\n\nsua conta em ls-rp.com.br.\n\n\n{FF0000}Tentativas restantes: %d", LoginAttempts[extraid]);
				return Dialog_Show(extraid, LoginScreen, DIALOG_STYLE_PASSWORD, "Bem vindo ao Los Santos Roleplay", str, "Avançar", "Sair");
			}
			else
			{
				...
				if(isnull(AccountData[extraid][aSecretWord]))
				{
			 		Dialog_Show(extraid, AddSecretWord, DIALOG_STYLE_PASSWORD,
						"Bem vindo ao Los Santos Roleplay",
						"{FFFFFF}PRECAUÇÃO DE SEGURANÇA:\n\nIntroduzimos um CÓDIGO DE CONFIRMAÇÃO SECRETO, que é basicamente uma palavra secreta que será apresentada a você caso haja alguma alteração nas condições de conexão.\n\n\
						Você ainda não preencheu isso, então por favor, reserve um tempo para preencher uma palavra secreta de sua escolha {FF0000}você precisará lembrar dessa palavra secreta.\n\n{FFFFFF}É RECOMENDADO QUE ISSO NÃO SEJA A SUA SENHA!",
						"Avançar", "Cancelar"
					);
				}
				else
				{
					...
					{
				 		Dialog_Show(extraid, SecretWord, DIALOG_STYLE_PASSWORD, "Bem vindo ao Los Santos Roleplay", "{FF0000}POSSÍVEL VIOLAÇÃO DE SEGURANÇA\n\n{FFFFFF}The server has flagged you as possibly not being this account's owner, please write the secret word of this account in the box below to confirm it is yours.", "Avançar", "Cancelar");
					}
					else
					{
		...
		case THREAD_LOAD_CHARACTER:
		{
			...
			if(rows)
			{
				...
				{
					SendClientMessageEx(extraid, COLOR_WHITE, "SERVER: Você logou como admin level %d", PlayerData[extraid][pAdmin]);
				}

	            SendClientMessageEx(extraid, COLOR_WHITE, "SERVER: Bem vindo(a) %s", ReturnName(extraid, 0));
				...
				if(PlayerData[extraid][pTimeout])
				{
					if(masked)
					{
						SendNoticeMessage(extraid, "Sua máscara foi devolvida (/mask)."); //Your mask has been returned
					...
					if(adminduty)
					{
						SendAdminAlert(COLOR_YELLOW, JUNIOR_ADMINS, "AdmCmd: %s voltou e está de volta em trabalho.", ReturnName(extraid));
					...
					if(testerduty)
					{
						SendTesterAlert(TEAM_TESTER_COLOR, "[TESTER] {FFBB00}%s retornou e está de volta ao trabalhoo de tester.", ReturnName(extraid));
			    ...
			    format(str, sizeof(str), "*** %s entrou no servidor", ReturnName(extraid));
	    ...
	    case THREAD_SECRET_CONFIRM:
		{
    	    ...
    	    if(!rows)
    	    {
                Dialog_Show(extraid, SecretWord, DIALOG_STYLE_PASSWORD, "Bem-vindo ao Los Santos Roleplay", "{FF0000}POSSÍVEL FALHA DE SEGURANÇA\n\n{FFFFFF}O servidor identificou que você possivelmente não é o proprietário desta conta, por favor escreva a palavra secreta desta conta na caixa abaixo para confirmar que é sua.", "Confirmar", "Cancelar");
			}
			else
			{
		    	SendClientMessage(extraid, COLOR_YELLOW, "Sua conexão com esta conta foi autenticada.");
		...
		case THREAD_CREATE_CHARACTER:
		{
		    ...
		    {
		        SendClientMessage(extraid, COLOR_LIGHTRED, "Algo deu errado, entre em contato com um administrador.");
		        return KickEx(extraid);
			}
		...
		case THREAD_CHECK_CHARACTER:
		{
		    ...
		    if(!rows)
			{
			    if(RegistrationEnabled)
					return Dialog_Show(extraid, CreateCharacter, DIALOG_STYLE_INPUT, "Criação de Personagem", "Como você não criou nenhum personagem, você pode criar um no jogo.\n\nVocê pode criar um agora mesmo digitando o nome que deseja usar abaixo\n\n{FF0000}**Certifique-se de digitar um nome realista ou você será banido sem nenhuma pergunta**", "Criar", "Sair");

				SendErrorMessage(extraid, "Você não tem nenhum personagem criado, acesse o UCP para criar.");
				return KickEx(extraid);
			}
				...
				if(!PlayerData[extraid][pLoginNotify])
				{
					foreach (new p : Player)
					{
						...
						if(AccountData[p][aUserid] == Friends[extraid][i][friendID])
						{
							...
							SendClientMessageEx(p, COLOR_GREY, "** %s está {33AA33}online{AFAFAF}! (%s, ID %d)", AccountData[extraid][aUsername], ReturnName(extraid), extraid);
						}
					}
				}
			}
